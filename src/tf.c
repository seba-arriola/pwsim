...
/* Inverts a complex square matrix using Gauss-Jordan elimination.
 * The base of this code was taken from codesansar.com and modified for
 * an arbitrary complex matrix size. */
...
double complex ** SATF_P_SV(double *freq,int freq_size,double *vs_sta,double *vp_sta,double *espesor_sta,int model_size,double damping_p, double damping_sv,double angle,double rho_satf)
{
    ...
    double w;
    double tp=angle;
    double betaS = damping_sv;
    double betaP = damping_p;
    double kx,kz,k;
    double complex **K;
...
double complex * SATF_SH(double *freq,int freq_size,double *vs_sta,double *espesor_sta,int model_size,double damping_sh, double rho_satf)
{
    ...
    for(i=0;i<model_size-1;i++)
    {
        ...
        delta[i]=csqrt((vs_sta[i]*vs_sta[i]*(1.+I*2.*damping_sh))/(vs_sta[i+1]*vs_sta[i+1]*(1.+I*2.*damping_sh)));
        ...
        for(j=0;j<freq_size;j++)
        {
            ...
            E[j]=1.0;
            F[j]=1.0;
            ka[i][j]= (2.*M_PI*freq[j])/(vs_sta[i]*csqrt(1.+I*2.*damping_sh));
